...
**OAuth2로 인증한 사용자들이 채팅방에 참여하고, STOMP Pub/Sub로 실시간 메시지를 주고받으며, 메시지는 DB에 저장**됩니다.
...
- **메시지**
  ...
  - 메시지 DB 저장 및 조회
- **새 메시지 표시**
  - 방 별 `lastCheckedAt` 기준으로 **내가 확인 이후 올라온 메시지 존재 여부** 계산
- **인증/권한**
  - 사용자: **OAuth2 로그인(카카오/구글)**
  - 상담사: **폼 로그인 + ROLE_CONSULTANT만 접근 허용**

---

## 2) 환경 구성 설명

### 2-1. WebSocket + STOMP(Pub/Sub)
- 채팅은 HTTP처럼 “요청-응답”보다 **서버→클라이언트 푸시**가 핵심
- STOMP를 사용시:
  - “채팅방 별 토픽”을 **구독(subscribe)** 하는 구조가 자연스럽고
  - 클라이언트는 **/pub** 으로 발행(publish), 서버/브로커가 **/sub** 구독자에게 브로드캐스트하는 형태로 확장성 용이
- WebSocket 핸들러:
  - **WebSocketHandler 방식 vs STOMP 방식** 비교/학습이 가능

### 2-2. Spring Security + OAuth2
- 채팅은 “누가 보냈는지”가 중요해서 **인증된 사용자 식별**이 필수
- OAuth2:
  - 사용자 비밀번호를 직접 다루지 않고 외부 인증(카카오/구글) 기반으로 빠르게 회원 유입이 가능
- 상담사:
  - OAuth2가 아니라 **폼 로그인 + ROLE 기반 접근 제어**가 더 직관적

### 2-3. JPA + MySQL
- 채팅방/사용자/메시지/참여관계는 전형적인 관계형 모델
- JPA:
  - 엔티티 중심으로 빠르게 구현하고,
  - 저장/조회 로직을 Repository로 분리해 유지보수성을 확보
- MySQL은 로컬 개발 환경에서 접근성이 좋고 운영 환경 이식 가능
...
- **Java 17 / Spring Boot 3.3**
  - 최신 기반으로 안정적이고, Spring 생태계와 호환성이 용이
- **Spring Web**
